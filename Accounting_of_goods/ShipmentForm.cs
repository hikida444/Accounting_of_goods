	...
	public partial class ShipmentForm : Form
	{
		...
		private void ShipmentForm_Load(object sender, EventArgs e)
		{
			if (dgvCart.Columns.Count == 0)
			{
				...
				dgvCart.Columns.Add("ProductName", "Товар");
		...
		private void cmbProduct_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			using (ApplicationDbContext db = new ApplicationDbContext())
			{
				var availableSizes = db.Products
					.Where(p => p.Name == selectedProductName)
					.Select(p => new { p.Id, p.Size, p.CurrentStock })
					.ToList();
		...
		private void btnAddToCart_Click(object sender, EventArgs e)
		{
			...
			int productId = (int)cmbSize.SelectedValue;
			...
			dgvCart.Rows.Add(productId, productName, size, qty);
			textBox1.Clear();
			textBox1.Focus();
